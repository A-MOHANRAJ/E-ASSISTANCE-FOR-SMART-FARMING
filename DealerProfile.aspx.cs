...
{
    public partial class DealerProfile : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                else
                {
                    ...
                    if (Request.QueryString["mode"].Equals("predit") && !string.IsNullOrEmpty(Session.SessionID) && Session["Master_Type_ID"].ToString().Equals("3"))
                    {
                        GetStateDetails();
                        GetFarmerInfo();
        ...
        private void GetFarmerInfo()
        {
            ...
            string query;
            query = "select * from dealer_user_details fa inner join master_state_details st on fa.State_ID=st.Id inner join master_district_details dt on fa.Id=dt.Id where fa.IsActive=True and fa.Id=" + Session["Id"].ToString();
            ...
            if (dtResult.Rows.Count > 0)
            {
                ...
                txtOffName.Text = dtResult.Rows[0]["Office_Name"].ToString();
                txtOffAddress.Text= dtResult.Rows[0]["Office_Address"].ToString();
