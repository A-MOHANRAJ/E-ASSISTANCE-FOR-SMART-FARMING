...
namespace Onliine_Agro_Products_System
{
    ...
    public partial class FarmerProducts : System.Web.UI.Page
    {
        ...
        private void LoadDataList(string mode,string value)
        {
            ...
            {
                qry = "select unt.Unit_Name as Unit,crp.Crop_Name,fp.Qty,fp.ToSale,st.State_Name,fud.Name,fud.Address,fud.Contact_No,fud.Email," +
                     "fp.Remarks,date_format(fp.created_on,'%d-%m-%y') as Posted_On,fp.Id as FP_ID,unt.Id as UNT_ID from " +
                     "master_state_details st inner join farmer_user_details fud on st.Id = fud.State_ID inner join farmer_product_details " +
                     "fp on fud.Id = fp.Farmer_ID inner join master_crop_details " +
                     "crp on fp.Crop_ID = crp.Id inner join master_unit_details unt on fp.Unit_ID=unt.Id where fp.ToSale > 0 order by fp.Created_On desc";
            }
            else if (mode.Equals("state"))
            {
                qry = "select unt.Unit_Name as Unit,crp.Crop_Name,fp.Qty,fp.ToSale,st.State_Name,fud.Name,fud.Address,fud.Contact_No,fud.Email," +
                     ...
                     "master_state_details st inner join farmer_user_details fud on st.Id = fud.State_ID inner join farmer_product_details " +
                     "fp on fud.Id = fp.Farmer_ID inner join master_crop_details " +
                     "crp on fp.Crop_ID = crp.Id inner join master_unit_details unt on fp.Unit_ID=unt.Id where fp.ToSale > 0 and st.id='"+value.ToString()+"' order by fp.Created_On desc";
           ...
           //Sort by Crop Name
                ...
                qry = "select unt.Unit_Name as Unit,crp.Crop_Name,fp.Qty,fp.ToSale,st.State_Name,fud.Name,fud.Address,fud.Contact_No,fud.Email," +
                         ...
                         "master_state_details st inner join farmer_user_details fud on st.Id = fud.State_ID inner join farmer_product_details " +
                         "fp on fud.Id = fp.Farmer_ID inner join master_crop_details " +
                         "crp on fp.Crop_ID = crp.Id inner join master_unit_details unt on fp.Unit_ID=unt.Id where fp.ToSale > 0 and crp.id='" + value.ToString() + "' order by fp.Created_On desc";
            ...
            try
            {
                ...
                else
                {
                    ...
                    dlProduct.DataBind();
                    lblmsg.Text = "Sorry,Currently No Products Available!!";
        ...
        protected void dlProduct_ItemCommand(object source, DataListCommandEventArgs e)
        {
            if (e.CommandName == "request") {
                ...
                {
                    lblmsg.Text = "Required qty should not be greater than Available";
                    lblmsg.ForeColor = System.Drawing.Color.Red;
                    ...
                    fieldValueList.Add("Farmer_Req_ID", fp_Id);
                    fieldValueList.Add("Dealer_ID", dud_Id);
                    fieldValueList.Add("Sold_Qty", req_Qty.ToString());
                    fieldValueList.Add("Unit_ID", unt_Id);
                    resultList = businessLogic.InsertRecord(ConfigurationManager.AppSettings["Farmer_Transaction_Info"].ToString(), fieldValueList);
                    if (resultList.Count > 1)
                    {
                        lblmsg.Text = "Product Request Sent Successfully";
