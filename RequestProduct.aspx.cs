    ...
    public partial class RequestProduct : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    if (Session != null && Session["Master_Type_ID"].Equals("3"))
                    {
                        GetCropDetails();
                        GetUnitDetails();
                    }
        ...
        protected void btnRegister_Click(object sender, EventArgs e)
        {
            ...
            fieldValueList.Add("Dealer_ID", "'" + Session["Id"].ToString() + "'");
            fieldValueList.Add("Crop_ID", "'" + ddlCrop.SelectedItem.Value + "'");
            fieldValueList.Add("Request_Qty", "'" + txtQty.Text + "'");
            fieldValueList.Add("Unit_ID", "'" + ddlUnit.SelectedItem.Value + "'");
            fieldValueList.Add("Status_Id", "4");
            fieldValueList.Add("Remarks", "'" + txtRemarks.Text + "'");
            fieldValueList.Add("IsActive", "True"); 
            fieldValueList.Add("ToBuy", "'" + txtQty.Text + "'");
            resultList = businessLogic.InsertRecord(ConfigurationManager.AppSettings["Dealer_Requirment_Info"].ToString(), fieldValueList);
            if (resultList.Count > 1)
            {
                lblMsg.Text = "Request posted successfully";
